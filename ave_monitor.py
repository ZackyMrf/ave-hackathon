...
class AveAccumulationMonitor:
    ...
    def _normalize_token_items(self, payload: object) -> List[Dict]:
        """Normalize token payloads into list[dict]."""
    ...
    def _dedupe_candidate_items(self, items: List[Dict], limit: int) -> List[Dict]:
        """Dedupe candidates by symbol/address while preserving strongest market caps."""
    ...
    def _filter_candidates_by_category(self, candidates: List[Dict], category: str) -> List[Dict]:
        """Apply optional category filter over chain-wide candidates."""
    ...
    def get_token_data(self, token: str, chain: str) -> Optional[TokenData]:
        ...
        # Try to get address from multiple sources with priority
        token_address = token  # Default fallback
        lookup_key = f"{token.lower()}-{chain.lower()}"
        
        # 1. Check cached TOKEN_ADDRESS_MAP first
        if lookup_key in TOKEN_ADDRESS_MAP:
            token_address = TOKEN_ADDRESS_MAP[lookup_key]
        
        # 2. If not cached, search endpoint to resolve symbol→address
        if token_address == token:  # Still default, didn't find it
            search_data = self._search_token(token, chain)
            if search_data and "token" in search_data:
                token_address = search_data["token"]
        ...
        # Extract final address (prefer cached, then search result, then API)
        extracted_address = (
            TOKEN_ADDRESS_MAP.get(lookup_key) or  # Cached known addresses
            token_address or  # From search
            data.get("address") or  # From API
            data.get("token", {}).get("address") or  # Nested
            token  # Fallback to symbol
        )
